...
void intersect_leaf_cylinder( bool& valid_isect, float4& isect, const quad& q0, const quad& q1, const float t_min, const float3& ray_origin, const float3& ray_direction )
{
    ...
    if( t_near  > t_min && t_near_ok  && t_near  < t_cand ) t_cand = t_near ;
    if( t_far   > t_min && t_far_ok   && t_far   < t_cand ) t_cand = t_far ;
    if( t_z1cap > t_min && t_z1cap_ok && t_z1cap < t_cand ) t_cand = t_z1cap ;
    if( t_z2cap > t_min && t_z2cap_ok && t_z2cap < t_cand ) t_cand = t_z2cap ;
    if( t_phi1  > t_min && t_phi1_ok  && t_phi1  < t_cand ) t_cand = t_phi1 ;
    if( t_phi2  > t_min && t_phi2_ok  && t_phi2  < t_cand ) t_cand = t_phi2 ;
