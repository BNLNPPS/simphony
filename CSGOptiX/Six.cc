...
Six::Six(const char *ptx_path_, const char *geo_ptx_path_, Params *params_)
    : emm(SGeoConfig::EnabledMergedMesh()), context(optix::Context::create()), material(context->createMaterial()),

      d_pixel(nullptr), d_isect(nullptr), d_photon(nullptr),

      params(params_), ptx_path(strdup(ptx_path_)), geo_ptx_path(strdup(geo_ptx_path_)), entry_point_index(0u),
      optix_device_ordinal(0u), foundry(nullptr), pindex(SSys::getenvint("PINDEX", 0))
{
    initContext();
    initPipeline();
    initFrame();
...
void Six::initFrame()
{
    ...
    LOG(info) << " params->width " << params->width << " params->height " << params->height << " optix_device_ordinal "
              << optix_device_ordinal;
}
...
{
    LOG(info) << " params.cameratype " << params->cameratype << " params.tmin " << params->tmin;
...
template <typename T> void Six::createContextInputBuffer(T *d_ptr, unsigned num_item, const char *name)
{
...
optix::Geometry Six::createGeometry(unsigned solid_idx)
{
    ...
    LOG(info) << " solid_idx " << std::setw(3) << solid_idx << " numPrim " << std::setw(3) << numPrim << " primOffset "
              << std::setw(3) << primOffset << " d_pr " << d_pr;
...
optix::Group Six::createIAS(unsigned ias_idx)
{
    ...
    LOG(info) << " ias_idx " << ias_idx << " tot_inst " << tot_inst << " num_inst " << num_inst << " emm(hex) "
              << std::hex << emm << std::dec;
    assert(num_inst > 0);
