    ...
    void UserSteppingAction(const G4Step *aStep)
    {
        ...
        if (stepStatus != fAtRestDoItProc)
        {
            ...
            for (size_t i3 = 0; i3 < MAXofPostStepLoops; i3++)
            {
                ...
                // Diagnostic (Step 4 fold-in): once per worker, dump the optical processes
                // we see to verify G4OpticalPhysics installed the Quasi variants and not
                // the legacy ones. Removed in Step 8.
                ...
                {
                    G4cout << "[GPURaytraceQuasi] post-step process found: " << procName << G4endl;
                    if (procName == "Cerenkov" || procName == "Scintillation")
                    {
                        G4cout << "[GPURaytraceQuasi] WARNING: legacy " << procName
                ...
                if (procName == "QuasiCerenkov")
                {
                    ...
                    if (fNumPhotons > 0)
                    {
                        ...
                        if (!quasi_dump_done)
                        {
                            G4cout << "[GPURaytraceQuasi] first QuasiCerenkov genstep captured: N="
                                   << fNumPhotons << G4endl;
                ...
                if (procName == "QuasiScintillation" || procName == "QausiScintillation")
                {
                    ...
                    if (fNumPhotons > 0)
                    {
                        ...
                        if (!quasi_dump_done)
                        {
                            G4cout << "[GPURaytraceQuasi] first QuasiScintillation genstep captured: N="
                                   << fNumPhotons << G4endl;
                            quasi_dump_done = true;
                        }
                    }
