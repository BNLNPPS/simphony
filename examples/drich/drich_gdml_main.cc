...
{
public:
    DRICH_DummySD(const G4String& name) : G4VSensitiveDetector(name) {}
...
struct DRICH_SensorIdentifier : public U4SensorIdentifier
{
    ...
    int getGlobalIdentity(const G4VPhysicalVolume* pv, const G4VPhysicalVolume*) override {
        ...
        if (!lv) return -1;
        if (gSensorLVs.count(lv) == 0) return -1;
...
static void AttachSDsToOpticalEfficiencySurfaces()
{
    ...
    for (size_t i = 0; i < store->size(); ++i) {
        ...
        if (!lv) continue;
        if (lv->GetSensitiveDetector()) continue;
        ...
        if (!eff || eff->GetVectorLength() == 0) continue;
        bool any_nonzero = false;
        for (G4int j = 0; j < eff->GetVectorLength(); ++j) {
            if ((*eff)[j] > 0.) { any_nonzero = true; break; }
        }
...
{
public:
    DRICH_GDML_Detector(const char* gdml) : fGdml(gdml) {}
...
public:
    ...
    void GeneratePrimaries(G4Event* evt) override {
        static int trace_evt = std::getenv("STEP_TRACE_EVT")
            ? std::atoi(std::getenv("STEP_TRACE_EVT")) : -1;
...
public:
    G4ClassificationOfNewTrack ClassifyNewTrack(const G4Track* tr) override {
        if (gKillOptical && tr->GetDefinition() == G4OpticalPhoton::Definition()) return fKill;
        return fUrgent;
    }
};
...
public:
        ...
        if (auto* g = G4ParticleTable::GetParticleTable()->FindParticle("opticalphoton")) {
            if (auto* pm = g->GetProcessManager()) {
                ...
                for (G4int i = 0; pvec && i < pvec->size(); ++i)
                    if (auto* c = dynamic_cast<G4OpBoundaryProcess*>((*pvec)[i])) { fBP = c; break; }
            }
        }
    }
    void UserSteppingAction(const G4Step* step) override {
        ...
        static int trace_evt_sa = std::getenv("STEP_TRACE_EVT")
            ? std::atoi(std::getenv("STEP_TRACE_EVT")) : -1;
        if (trace_evt_sa >= 0 && gCurrentEventID == trace_evt_sa
            && (gStepsAll % 100000) == 0)
        {
        ...
        static long trace_all_after = std::getenv("STEP_TRACE_ALL_AFTER")
            ? std::atol(std::getenv("STEP_TRACE_ALL_AFTER")) : -1;
        if (trace_all_after >= 0 && trace_evt_sa >= 0
            && gCurrentEventID == trace_evt_sa
            && gStepsAll >= trace_all_after)
        {
        ...
        static int max_step_watchdog = std::getenv("MAX_STEP_WATCHDOG")
            ? std::atoi(std::getenv("MAX_STEP_WATCHDOG")) : 100000;
        ...
        static int stuck_diag = std::getenv("STUCK_DIAG")
            ? std::atoi(std::getenv("STUCK_DIAG")) : 0;
        ...
        static int trace_min = std::getenv("STEP_TRACE_MIN")
            ? std::atoi(std::getenv("STEP_TRACE_MIN")) : 100;
        static int trace_every = std::getenv("STEP_TRACE_EVERY")
            ? std::atoi(std::getenv("STEP_TRACE_EVERY")) : 1;
        if (trace_evt >= 0 && gCurrentEventID == trace_evt) {
            ...
            static long trace_tick = std::getenv("STEP_TRACE_TICK")
                ? std::atol(std::getenv("STEP_TRACE_TICK")) : 1000000;
        ...
        if (gKillOptical) return;
        if (track->GetDefinition() != G4OpticalPhoton::Definition()) return;
        ++gStepsOptical;

        if (!fBP) cacheBoundary();
        G4OpBoundaryProcess* bp = fBP;

        if (!bp) {
            static bool warned = false;
            if (!warned) { std::cerr << "[diag] NO G4OpBoundaryProcess on opticalphoton process list!\n"; warned = true; }
            return;
        }
        ...
        if (status == Undefined || status == NotAtBoundary) return;
        ++gStepsOpticalBoundary;

        if (status == Detection) ++gCpuOpDetectThisEvent;
        if (status == Absorption) ++gCpuOpAbsorbThisEvent;
        if (status == FresnelReflection || status == TotalInternalReflection
...
public:
    void BeginOfEventAction(const G4Event* evt) override {
        ...
        if (sev) sev->beginOfEvent(evt->GetEventID());
        if (trace_evt >= 0 && evt->GetEventID() == trace_evt)
        ...
        gCpuHitX.clear(); gCpuHitY.clear(); gCpuHitZ.clear(); gCpuHitWL.clear();
    }
    void EndOfEventAction(const G4Event* evt) override {
        ...
        static int trace_evt_ev = std::getenv("STEP_TRACE_EVT")
            ? std::atoi(std::getenv("STEP_TRACE_EVT")) : -1;
        if (trace_evt_ev >= 0 && evt->GetEventID() == trace_evt_ev
            && !gMaxStepPerTrack.empty())
        {
            ...
            const bool dump_all = std::getenv("STEP_TRACE_DUMPALL")
                && std::atoi(std::getenv("STEP_TRACE_DUMPALL")) != 0;
...
{
public:
    DRICH_RunAction(const char* gdml_path) : fGdmlPath(gdml_path) {}
        ...
        if (std::getenv("DUMP_BND")) {
            ...
            if (bnd && op) {
                ...
                int lhalf = std::getenv("DUMP_BND_LBIN")
                    ? std::atoi(std::getenv("DUMP_BND_LBIN")) : nl/2;
                int boundary_to_dump = std::getenv("DUMP_BND_IDX")
                    ? std::atoi(std::getenv("DUMP_BND_IDX")) : -1;
                for (int i = 0; i < ni; ++i) {
                    if (boundary_to_dump >= 0 && i != boundary_to_dump) continue;
        ...
        if (auto* gamma = G4ParticleTable::GetParticleTable()->FindParticle("opticalphoton")) {
            if (auto* pm = gamma->GetProcessManager()) {
                ...
                for (G4int i = 0; pv && i < pv->size(); ++i)
                    if ((*pv)[i]) (*pv)[i]->SetVerboseLevel(0);
            }
        }
...
int main(int argc, char** argv)
{
    ...
    if (const char* s = std::getenv("KILL_OPTICAL")) gKillOptical = std::atoi(s) != 0;
    if (const char* s = std::getenv("DUMP_HITS"))    gDumpHits    = std::atoi(s) != 0;
    ...
    {
        const char* pname = std::getenv("PARTICLE"); if(!pname||!*pname) pname="mu-";
        const char* pmom  = std::getenv("MOMENTUM_GEV");
        std::cout << "[main] gun: " << pname << "  " ;
        if (pmom) std::cout << "p=" << pmom << " GeV/c";
        else      std::cout << "KE=" << E << " GeV";
