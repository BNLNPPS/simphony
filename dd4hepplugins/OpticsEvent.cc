...
//---------------------------------------------------------------------------//
OpticsEvent::~OpticsEvent() { dd4hep::InstanceCount::decrement(this); }
...
void OpticsEvent::end(G4Event const *event) {
  ...
  if (verbose_ > 0 || num_genstep > 0) {
    info("Event #%d: %lld gensteps, %lld photons accumulated", eventID,
  ...
  if (num_genstep > 0) {
    ...
    info("OPTICKS_GPU_TIME event=%d ms=%.3f photons=%lld hits=%u", eventID,
         simulate_ms, static_cast<long long>(num_photon), num_hit);
...
//---------------------------------------------------------------------------//
void OpticsEvent::injectHits(G4Event const *event, SEvt *sev,
                             unsigned num_hit) {
  ...
  for (auto const &[det_name, seq] : seqs) {
    ...
    info("Event #%d: injected %u hits into '%s' collection", eventID, num_hit,
         det_name.c_str());
    break;
  }
}
