...
/**
 * Captures the current CPU and GPU genstep counts.
 *
 * `assertAdded` verifies that a subsequent collection added one genstep.
 */
struct U4Recorder_SEvtGenstepCount
{
    ...
    {
        if (SEvt::Exists(SEvt::EGPU))
            assert(SEvt::GetNumGenstepFromGenstep(SEvt::EGPU) == gpu + 1);
        if (SEvt::Exists(SEvt::ECPU))
            assert(SEvt::GetNumGenstepFromGenstep(SEvt::ECPU) == cpu + 1);
    }
};

/**
 * Selects the secondaries created by one process during the current step.
 *
 * @param step step whose current secondaries are inspected
 * @param process creator process used to select secondaries
 * @return selected secondaries in Geant4 creation order
 */
std::vector<const G4Track*> U4Recorder_Secondaries(const G4Step* step, const G4VProcess* process)
{
    ...
    {
        if (secondary && secondary->GetCreatorProcess() == process)
...
/**
 * Assigns one genstep's photon labels to a contiguous secondary range.
 *
 * @param parent charged parent track that produced the secondaries
 * @param secondaries ordered secondary tracks for the current process
 * @param cursor index of the first secondary, advanced by the number labeled
 * @param count number of secondaries belonging to the genstep
 */
...
/**
 * Reconstructs Opticks gensteps after the official Geant4 Cerenkov and
...
/**
 * Labels optical secondaries created by the official Geant4 WLS process.
 *
