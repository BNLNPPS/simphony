...
struct ssys
{
    ...
    template <typename T> static T parse(const char *str);

    template <typename T> static T getenv_(const char *ekey, T fallback);

    template <typename T>
    static void getenv_(std::vector<std::pair<std::string, T>> &kv, const std::vector<std::string> &kk);

    template <typename T> static void getenv_(std::vector<std::pair<std::string, T>> &kv, const char *kk);

    template <typename T> static void fill_vec(std::vector<T> &vec, const char *line, char delim = ',');

    template <typename T>
    static void fill_evec(std::vector<T> &vec, const char *ekey, const char *fallback, char delim);

    template <typename T> static std::vector<T> *make_vec(const char *line, char delim = ',');

    template <typename T> static std::vector<T> *getenv_vec(const char *ekey, const char *fallback, char delim = ',');
    ...
    template <typename T> static std::string desc_vec(const std::vector<T> *vec, unsigned edgeitems = 5);
    ...
    static int setenvmap(const std::map<std::string, std::string> &env, bool overwrite = true,
                         char special_empty_token = '\0');

    template <typename... Args> static int setenvctx(Args... args);
...
inline std::string ssys::which(const char *script)
{
    ...
    if (VERBOSE)
        std::cerr << " script " << script << " path " << path << " rc " << rc << std::endl;
...
{
    return val && strlen(val) > strlen(GETENVVAR_PATH_PREFIX) &&
           strncmp(val, GETENVVAR_PATH_PREFIX, strlen(GETENVVAR_PATH_PREFIX)) == 0;
}
...
inline const char *ssys::getenvvar(const char *ekey)
{
    ...
    */
    ...
    if (is_path_prefixed)
    {
        ...
        if (VERBOSE)
            std::cout << "ssys::getenvvar.is_path_prefixed " << " ekey " << (ekey ? ekey : "-") << " path "
                      << (path ? path : "-") << " path_exists " << (path_exists ? "YES" : "NO ") << " val "
                      << (val ? val : "-") << std::endl;
    }
    return val;
}
...
inline const char *ssys::getenvvar(const char *ekey, const char *fallback, char q, char r)
{
    ...
    for (int i = 0; i < int(vv ? strlen(vv) : 0); i++)
        if (vv[i] == q)
            vv[i] = r;
    return vv;
}
...
inline int64_t ssys::getenv_ParseInt64(const char *ekey, const char *fallback)
{
    ...
    if (!valid)
    {
        std::cerr << "ssys::getenv_ParseInt64" << " ekey " << (ekey ? ekey : "-") << " fallback "
                  << (fallback ? fallback : "-") << " spec [" << (spec ? spec : "-") << "]" << " valid "
                  << (valid ? "YES" : "NO ") << "\n";
...
inline float ssys::getenvfloat(const char *ekey, float fallback)
{
    return getenv_<float>(ekey, fallback);
}
inline double ssys::getenvdouble(const char *ekey, double fallback)
{
    return getenv_<double>(ekey, fallback);
}
...
template <typename T> inline T ssys::parse(const char *str_)
{
    std::string str(str_);
...
template <> inline std::string ssys::parse(const char *str_)
{
...
template <typename T> inline T ssys::getenv_(const char *ekey, T fallback)
{
    char *v = getenv(ekey);
...
template <typename T> void ssys::getenv_(std::vector<std::pair<std::string, T>> &kv, const std::vector<std::string> &kk)
{
...
template <typename T> void ssys::getenv_(std::vector<std::pair<std::string, T>> &kv, const char *kk_)
{
...
template <typename T> inline void ssys::fill_vec(std::vector<T> &vec, const char *line, char delim)
{
    std::stringstream ss;
    ss.str(line);
    std::string s;
...
inline void ssys::fill_evec(std::vector<T> &vec, const char *ekey, const char *fallback, char delim)
{
    ...
    if (line_ == nullptr && fallback == nullptr)
        return;
...
template <typename T> inline std::vector<T> *ssys::make_vec(const char *line, char delim)
{
    if (line == nullptr)
        return nullptr;
...
template <typename T> inline std::vector<T> *ssys::getenv_vec(const char *ekey, const char *fallback, char delim)
{
...
template <typename T> inline std::string ssys::desc_vec(const std::vector<T> *vec, unsigned edgeitems)
{
    ...
    for (unsigned i = 0; i < size; i++)
        if (i < edgeitems || i > size - edgeitems)
            ss << (*vec)[i] << " ";
    ss << ")";
...
inline std::string ssys::desc_listed_count(const std::vector<int> *ncount, const std::vector<std::string> *nn)
{
    ...
    std::stringstream ss;
    ss << "ssys::desc_listed_count" << " ncount_sz " << ncount_sz << " nn_sz " << nn_sz << std::endl;

    if (ncount_sz == nn_sz && nn_sz > -1)
    {
        for (int i = 0; i < nn_sz; i++)
            ss << std::setw(3) << i << " : " << (*ncount)[i] << " : " << (*nn)[i] << std::endl;
    }
    std::string str = ss.str();
    return str;
}
...
inline int ssys::run(const char *cmd)
{
    ...
    std::cout << "ssys::run " << (cmd ? cmd : "-") << " rc_raw : " << rc_raw << " rc : " << rc << std::endl;

    if (rc != 0)
        std::cout << "ssys::run" << " PATH ENVVAR MISCONFIGURED ? " << std::endl;
    return rc;
}
...
inline int ssys::setenvvar(const char *ekey, const char *value, bool overwrite, char special_empty_token)
{
    ...
    if (VERBOSE)
        std::cerr << "ssys::setenvvar" << " ekey " << ekey << " ekv " << ekv << " overwrite " << overwrite << " prior "
                  << (prior ? prior : "NULL") << " value " << (value ? value : "NULL") << " after "
                  << (after ? after : "NULL") << " rc " << rc << std::endl;
...
template <typename... Args> inline int ssys::setenvctx(Args... args_)
{
...
template int ssys::setenvctx(const char *, const char *);
template int ssys::setenvctx(const char *, const char *, const char *, const char *);
template int ssys::setenvctx(const char *, const char *, const char *, const char *, const char *, const char *);
template int ssys::setenvctx(const char *, const char *, const char *, const char *, const char *, const char *,
