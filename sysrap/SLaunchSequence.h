...
{
    std::stringstream ss;
    ss << "SLaunch::desc" << " sequence_index " << std::setw(3) << sequence_index << " thread_offset " << std::setw(7)
       << thread_offset << " threads_per_launch " << std::setw(6) << threads_per_launch << " blocks_per_launch "
       << std::setw(6) << blocks_per_launch << " threads_per_block " << std::setw(6) << threads_per_block
...
inline SLaunchSequence::SLaunchSequence(unsigned items_)
    : items(items_), threads_per_block(ssys::getenvint("THREADS_PER_BLOCK", 512)),
...
inline void SLaunchSequence::init()
{
    ...
    while (thread_offset < items)
    {
        ...
        if (remaining % threads_per_block != 0)
            blocks_per_launch += 1;
        if (blocks_per_launch > max_blocks)
            blocks_per_launch = max_blocks;
        ...
        if (threads_per_launch > remaining)
            threads_per_launch = remaining;

        launches.push_back(
            {thread_offset, threads_per_launch, blocks_per_launch, threads_per_block, sequence_index, -1.f});
...
inline float SLaunchSequence::total_time() const
{
    ...
    for (unsigned i = 0; i < launches.size(); i++)
    {
        ...
        if (launch.kernel_time > 0.f)
            total += launch.kernel_time;
    }
    return total;
}
...
{
    std::stringstream ss;
    ss << " items " << std::setw(7) << items << " total_threads " << std::setw(7) << total_threads()
       << " THREADS_PER_BLOCK " << std::setw(5) << threads_per_block << " MAX_BLOCKS " << std::setw(6) << max_blocks
       << " num_launches " << std::setw(4) << launches.size() << " total_time " << std::setw(10) << std::fixed
...
{
    std::stringstream ss;
    ss << "SLaunchSequence::desc" << brief() << std::endl;
    for (unsigned i = 0; i < launches.size(); i++)
    {
        const SLaunch &launch = launches[i];
