...
const char *SPath::Resolve(const char *spec_, int create_dirs)
{
    ...
    LOG(LEVEL) << " spec_ [" << spec_ << "]" << " create_dirs [" << create_dirs << "]";
    ...
    char sep = '/';
    char *spec_sep =
        strchr(spec, sep); // pointer to first separator : so spec_sep is string starting from the first sep
    char *spec_end = strchr(spec, '\0'); // pointer to null terminator

    LOG(LEVEL) << " spec " << spec << " spec_sep " << spec_sep << " spec_end " << spec_end;
...
void SPath::CreateDirs(const char *path, int mode) // static
{
    ...
    if (rc != 0)
        std::cerr << "SPath::CreateDirs ERROR " << " path " << (path ? path : "-") << " mode " << mode << " dirname "
                  << (dirname ? dirname : "-") << " rc " << rc << std::endl;
...
{
    LOG(LEVEL) << " dir [" << dir << "]" << " name [" << (name ? name : "-") << "]" << " create_dirs [" << create_dirs
               << "]";
...
{
    LOG(LEVEL) << " dir [" << dir << "]" << " reldir [" << reldir << "]" << " name [" << name << "]" << " create_dirs ["
               << create_dirs << "]";
...
{
    LOG(LEVEL) << " dir [" << dir << "]" << " reldir [" << reldir << "]" << " rel2dir [" << rel2dir << "]" << " name ["
               << name << "]" << " create_dirs [" << create_dirs << "]";
...
const char *SPath::Resolve(const char *dir, const char *reldir, const char *rel2dir, const char *rel3dir,
                           const char *name, int create_dirs)
{
    LOG(LEVEL) << " dir [" << dir << "]" << " reldir [" << reldir << "]" << " rel2dir [" << rel2dir << "]"
               << " rel3dir [" << rel3dir << "]" << " name [" << name << "]" << " create_dirs [" << create_dirs << "]";
...
int SPath::MakeDirs(const char *path_, int mode_)
{
    ...
    while (*p != '\0' && rc == 0)
    {
        ...
        while (*p != '\0' && *p != '/')
            p++;     // advance p until subsequent slash
        char v = *p; // store the slash
        *p = '\0';   // replace slash with string terminator
        // printf("%s\n", path );
        rc = mkdir(path, mode) == -1 && errno != EEXIST
                 ? 1
                 : 0; // set rc non-zero for mkdir errors other than exists already
...
{
    if (SPath::Exists(path0))
        return path0;
    if (SPath::Exists(path1))
        return path1;
    if (SPath::Exists(path2))
        return path2;
    return nullptr;
}
...
void SPath::Copy(const char *dst__, const char *src__)
{
    ...
    if (same)
        std::cerr << "SPath::Copy : ERR SAME  " << std::endl;
    if (same)
        std::raise(SIGINT);
    assert(same == false);
...
const char *SPath::Make(const char *base, const char *reldir, const char *stem, int index, const char *ext,
                        int create_dirs)
{
...
const char *SPath::Make(const char *base, const char *reldir, const char *reldir2, const char *stem, int index,
                        const char *ext, int create_dirs)
{
    assert(create_dirs == NOOP || create_dirs == FILEPATH);
    std::string name = MakeName(stem, index, ext);
...
{
    if (startdir == nullptr || relf == nullptr)
        return nullptr;
    char *dir = strdup(startdir);
    while (dir && strlen(dir) > 1)
    {
        if (SPath::Exists(dir, relf))
            break;
...
template <typename... Args> std::string SPath::Join_(Args... args_)
{
...
template <typename... Args> const char *SPath::Join(Args... args)
{
