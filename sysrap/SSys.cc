...
void SSys::DumpEnv(const char *pfx) // static
{
    ...
    {
        if (pfx == NULL || strncmp(s, pfx, strlen(pfx)) == 0)
            printf("%s\n", s);
        s = *(environ + i);
    }
}
...
int SSys::run(const char *cmd)
{
    ...
    LOG(info) << cmd << " rc_raw : " << rc_raw << " rc : " << rc;

    if (rc != 0)
    {
        LOG(error) << "FAILED with " << " cmd " << cmd << " RC " << rc;
        LOG(verbose) << " possibly you need to set export "
                        "PATH=$OPTICKS_HOME/ana:$OPTICKS_HOME/bin:/usr/local/opticks/lib:$PATH ";
    }
    return rc;
}
...
std::string SSys::Which(const char *script)
{
    ...
    LOG(LEVEL) << " script " << script << " path " << path << " rc " << rc;
...
{
    if (!printoptions)
        printoptions = "suppress=True,precision=3";

    std::stringstream ss;
    ss << "python -c 'import sys, os, numpy as np ;" << " np.set_printoptions(" << printoptions << ") ;"
       << " a=np.load(os.path.expandvars(\"" << path << "\")) ;" << " print(a.shape);" << " print(a.view(" << nptype
       << ")" << (postview ? postview : "") << ") ;" << " sys.exit(0) ' ";
...
std::string SSys::hexlify(const void *obj, size_t size, bool reverse)
{
    ...
    for (size_t i = 0; i < size; i++)
        ss << std::setw(2) << std::hex << std::setfill('0') << unsigned(bytes[reverse ? size - 1 - i : i]);
    return ss.str();
}
...
bool SSys::IsVERBOSE()
{
    return IsENVVAR("VERBOSE");
}
bool SSys::IsHARIKARI()
{
    return IsENVVAR("HARIKARI");
}
...
bool SSys::IsRemoteSession()
{
    ...
    LOG(verbose) << "SSys::IsRemoteSession" << " ssh_client " << ssh_client << " ssh_tty " << ssh_tty << " is_remote "
                 << is_remote;
...
float3 SSys::getenvfloat3(const char *envkey, const char *fallback, char delim)
{
    ...
    return make_float3(fvec.size() > 0 ? fvec[0] : 0.f, fvec.size() > 1 ? fvec[1] : 0.f,
...
float4 SSys::getenvfloat4(const char *envkey, const char *fallback, char delim)
{
    std::vector<float> fvec;
    getenvfloatvec(envkey, fvec, fallback, delim);
    return make_float4(fvec.size() > 0 ? fvec[0] : 0.f, fvec.size() > 1 ? fvec[1] : 0.f,
                       fvec.size() > 2 ? fvec[2] : 0.f, fvec.size() > 3 ? fvec[3] : 0.f);
}
...
std::string SSys::Desc(const std::vector<float> *fvec)
{
    ...
    if (fvec == nullptr)
        ...
        for (unsigned i = 0; i < fvec->size(); i++)
            ss << std::setw(10) << std::fixed << std::setprecision(4) << (*fvec)[i] << " ";
    std::string s = ss.str();
    return s;
}
...
int SSys::setenvvar(const char *ekey, const char *value, bool overwrite, char special_empty_token)
{
    ...
    LOG(debug) << " ekey " << ekey << " ekv " << ekv << " overwrite " << overwrite << " prior "
               << (prior ? prior : "NULL") << " value " << (value ? value : "NULL") << " after "
               << (after ? after : "NULL") << " rc " << rc;
...
const char *SSys::ResolveExecutable(const char *envvar_key, const char *default_executable)
{
    ...
    if (ev && ev[0] == '/')
        return strdup(ev); // assume absolute path of python executable is provided
...
int SSys::RunPythonScript(const char *script)
{
    ...
    LOG(info) << " script " << script << " script_path " << script_path << " python_executable " << python_executable;
...
{
    const char *python_executable = SSys::ResolvePython();
    LOG(info) << " code [" << code << "]" << " python_executable " << python_executable;
