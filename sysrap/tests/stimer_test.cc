...
void test_convert_3()
{
    ...
    std::cout << " ec " << ec << " t0 " << stimer::Format(t0) << " ecf " << stimer::Format(ec) << std::endl;
}
...
void test_TimePoint_0()
{
    ...
    std::vector<uint64_t> ecs0 = {1681415567555006257, 1681415570432562093, 1681415573040370404, 1681415575667773937,
                                  1681415578398804926, 1681415581169340691, 1681415583816194221, 1681415586567097051,
                                  1681415589753517102, 1681415592426987792};

    std::vector<uint64_t> ecs1 = {1681415825234793213, 1681415828042329959, 1681415831150709245, 1681415833581851217,
                                  1681415836050784359, 1681415839417522516, 1681415842001793022, 1681415844497812015,
                                  1681415846965829828, 1681415850332486246};
    ...
    for (int p = 0; p < 2; p++)
    {
        for (int i = 0; i < int(ecs0.size()); i++)
        {
            ...
            if (p == 0)
                std::cout << " ec0 " << std::setw(20) << ec0 << " ec1 " << std::setw(20) << ec1 << std::endl;

            if (p == 1)
                std::cout << " ec0 " << stimer::Format(ec0) << " ec1 " << stimer::Format(ec1) << std::endl;
        }
    }
}
...
void test_EpochCountNow_0()
{
    ...
    {
        uint64_t ecn = ecns[i];
        std::cout << " stimer::Format(" << ecn << ") : " << stimer::Format(ecn) << std::endl;
    }
}
...
void test_count()
{
    ...
    std::cout << " stimer::Format(t0) " << stimer::Format(t0) << " t0 " << t0 << std::endl
...
From BST (UTC+1) timezone the numpy presented times are corrected to UTC::
    ...
    epsilon:tests blyth$ ./stimer_test.sh
    stimer::desc status STOPPED _start 1681480063800788 start Fri, 14.04.2023 14:47:43 _stop 1681480064801308 stop
Fri, 14.04.2023 14:47:44 duration 1.000520e+00 stimer::Format(t0) Fri, 14.04.2023 14:47:43 t0 1681480063800788
...
From CST (UTC+8) timezone the numpy presented times also corrected to UTC::
    ...
    N[blyth@localhost tests]$ ./stimer_test.sh
    stimer::desc status STOPPED _start 1681480305503105 start Fri, 14.04.2023 21:51:45 _stop 1681480306505512 stop
Fri, 14.04.2023 21:51:46 duration 1.002406e+00 stimer::Format(t0) Fri, 14.04.2023 21:51:45 t0 1681480305503105
