...
int sdigest_test::compare(const char *label, const std::vector<std::string> &dig, const char *known)
{
    ...
    {
        std::cout << " i " << std::setw(3) << i << " : " << dig[i] << std::endl;
...
int sdigest_test::hello()
{
    ...
    u2.add(dat, 5);
    for (int i = 0; i < 5; i++)
        u3.add(dat + i, 1); // adding character by character
...
int sdigest_test::main()
{
    ...
    int rc = 0;
    if (ALL || strcmp(TEST, "hello") == 0)
        rc += hello();
    if (ALL || strcmp(TEST, "int_") == 0)
        rc += int_();
    if (ALL || strcmp(TEST, "Desc") == 0)
        rc += Desc();
    if (ALL || strcmp(TEST, "Hit") == 0)
        rc += Hit();
