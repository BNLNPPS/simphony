...
struct SGLFW_Program
{
    ...
    SGLFW_Program(const char *_dir, const char *_vtx_attname, const char *_nrm_attname, const char *_ins_attname,
                  const char *_mvp_uniname, const float *_mvp);
    void init();

    void createFromDir(const char *_dir);
    void createFromText(const char *vertex_shader_text, const char *geometry_shader_text,
                        const char *fragment_shader_text);
    void use() const;
    ...
    template <typename T> static std::string Desc(const T *tt, int num);
};

inline SGLFW_Program::SGLFW_Program(const char *_dir, const char *_vtx_attname, const char *_nrm_attname,
                                    const char *_ins_attname, const char *_mvp_uniname, const float *_mvp)
    : level(ssys::getenvint(_LEVEL, 0)), dir(_dir ? strdup(_dir) : nullptr),
      ...
      mvp_uniname(_mvp_uniname ? strdup(_mvp_uniname) : nullptr), vertex_shader_text(nullptr),
      geometry_shader_text(nullptr), fragment_shader_text(nullptr), program(0), mvp_location(-1), mvp(_mvp), dump(false)
{
    init();
}
...
inline void SGLFW_Program::createFromDir(const char *_dir)
{
    ...
    if (level > 0)
        std::cout << "SGLFW_Program::createFromDir" << " _dir " << (_dir ? _dir : "-") << " dir " << (dir ? dir : "-")
                  << "\n";
    ...
    if (level > 0)
        std::cout << "SGLFW_Program::createFromDir" << " _dir " << (_dir ? _dir : "-") << " dir " << (dir ? dir : "-")
                  << " vertex_shader_text " << (vertex_shader_text ? "YES" : "NO") << " geometry_shader_text "
                  << (geometry_shader_text ? "YES" : "NO") << " fragment_shader_text "
                  << (fragment_shader_text ? "YES" : "NO") << std::endl;
...
inline void SGLFW_Program::createFromText(const char *vertex_shader_text, const char *geometry_shader_text,
                                          const char *fragment_shader_text)
{
    if (level > 0)
        std::cout << "[SGLFW_Program::createFromText level " << level << std::endl;
    if (level > 1)
        std::cout << " vertex_shader_text " << std::endl
                  << (vertex_shader_text ? vertex_shader_text : "-") << std::endl;
    if (level > 1)
        std::cout << " geometry_shader_text " << std::endl
                  << (geometry_shader_text ? geometry_shader_text : "-") << std::endl;
    if (level > 1)
        std::cout << " fragment_shader_text " << std::endl
                  << (fragment_shader_text ? fragment_shader_text : "-") << std::endl;
    ...
    if (geometry_shader_text)
    {
        ...
        if (GL_TRUE != params)
            Print_shader_info_log(geometry_shader);
    }
...
inline GLint SGLFW_Program::getUniformLocation(const char *name) const
{
    ...
    SGLFW__check(__FILE__, __LINE__, name, loc, "glGetUniformLocation/m");
    return loc;
}
...
inline GLint SGLFW_Program::getAttribLocation(const char *name) const
{
    ...
    SGLFW__check(__FILE__, __LINE__, name, loc, "glGetAttribLocation/m");
    return loc;
}
...
template <typename T> inline std::string SGLFW_Program::Desc(const T *tt, int num) // static
{
    std::stringstream ss;
    for (int i = 0; i < num; i++)
...
{
    if (dump)
        std::cout << "SGLFW_Program::UniformMatrix4fv" << " loc " << loc << std::endl << Desc(vv, 16) << std::endl;
...
{
    if (dump)
        std::cout << "SGLFW_Program::Uniform4fv" << " loc " << loc << std::endl << Desc(vv, 4) << std::endl;
...
inline void SGLFW_Program::enableVertexAttribArray(const char *name, const char *spec, bool dump) const
{
    ...
    att.index = getAttribLocation(name);
    SGLFW__check(__FILE__, __LINE__, name, att.index, "getAttribLocation");

    if (dump)
        std::cout << "SGLFW_Program::enableVertexAttribArray att.desc [" << att.desc() << "]" << std::endl;

    glEnableVertexAttribArray(att.index);
    SGLFW__check(__FILE__, __LINE__, name, att.index, "glEnableVertexAttribArray");

    assert(att.integer_attribute == false);

    glVertexAttribPointer(att.index, att.size, att.type, att.normalized, att.stride, att.byte_offset_pointer);
    SGLFW__check(__FILE__, __LINE__, name, att.index, "glVertexAttribPointer");
}
...
inline void SGLFW_Program::enableVertexAttribArray_OfTransforms(const char *name) const
{
    ...
    SGLFW__check(__FILE__, __LINE__, name, att.index + 0, "glEnableVertexAttribArray");
    glVertexAttribPointer(att.index + 0, att.size, att.type, att.normalized, att.stride, offset0);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 0, "glVertexAttribPointer");
    glVertexAttribDivisor(att.index + 0, divisor);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 0, "glVertexAttribDivisor");

    glEnableVertexAttribArray(att.index + 1);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 1, "glEnableVertexAttribArray");
    glVertexAttribPointer(att.index + 1, att.size, att.type, att.normalized, att.stride, offset1);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 1, "glVertexAttribPointer");
    glVertexAttribDivisor(att.index + 1, divisor);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 1, "glVertexAttribDivisor");

    glEnableVertexAttribArray(att.index + 2);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 2, "glEnableVertexAttribArray");
    glVertexAttribPointer(att.index + 2, att.size, att.type, att.normalized, att.stride, offset2);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 2, "glVertexAttribPointer");
    glVertexAttribDivisor(att.index + 2, divisor);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 2, "glVertexAttribDivisor");

    glEnableVertexAttribArray(att.index + 3);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 3, "glEnableVertexAttribArray");
    glVertexAttribPointer(att.index + 3, att.size, att.type, att.normalized, att.stride, offset3);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 3, "glVertexAttribPointer");
    glVertexAttribDivisor(att.index + 3, divisor);
    SGLFW__check(__FILE__, __LINE__, name, att.index + 3, "glVertexAttribDivisor");
}
