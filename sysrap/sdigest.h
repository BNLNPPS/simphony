...
struct sdigest
{
    ...
    static std::array<unsigned char, 16> ItemRaw(const NP *a, int i = -1, int j = -1, int k = -1, int l = -1,
                                                 int m = -1, int o = -1);
...
inline sdigest::sdigest()
{
    MD5_Init(&ctx);
}
inline void sdigest::add(const std::string &str)
{
    Update(ctx, str);
}
inline void sdigest::add(const char *str)
{
    Update(ctx, str);
}
inline void sdigest::add(int i)
{
    Update(ctx, i);
}
inline void sdigest::add(const char *str, int length)
{
    Update(ctx, str, length);
}
inline void sdigest::add(const std::vector<unsigned char> &bytes)
{
    Update(ctx, (char *)bytes.data(), bytes.size());
}

inline std::string sdigest::finalize()
{
    return Finalize(ctx);
}
inline std::array<unsigned char, 16> sdigest::finalize_raw()
{
    return FinalizeRaw(ctx);
}
...
inline std::string sdigest::Paths(std::vector<std::string> &paths, unsigned bufsize)
{
    ...
    for (int i = 0; i < num_paths; i++)
    {
        ...
        if (fp == NULL)
        {
            std::cerr << "sdigest::Paths" << " failed to open" << " path [" << path << "]\n";
            continue;
        }
...
template <typename T> inline void sdigest::Update_(MD5_CTX &c, T *vv, size_t count)
{
    Update(c, (char *)vv, sizeof(T) * count);
}

template <typename T> inline void sdigest::add_(T *vv, size_t count)
{
    Update_<T>(ctx, vv, count);
}
