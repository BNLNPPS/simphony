...
ana/nibble.py::
    ...
    def count_nibbles(x):
        ...
        count = (x * 0x101010101010101) >> 56     #  add up byte totals into top byte,  and shift that down to pole 64-8
= 56
...
template <typename T> std::string SBit::String(T v) // static
{
...
template <typename T> std::string SBit::BinString(T v, bool anno) // static
{
    ...
    std::stringstream ss;
    ss << (express_flipped ? "~" : " ") << (anno ? "0b" : "") << bs.to_string();
    ;
...
template <typename T> std::string SBit::HexString(T v, bool anno) // static
{
    std::bitset<64> bs(v);
    assert(bs.size() == 64);
    bool express_flipped = 2 * bs.count() > bs.size(); // more than half the bits are set
    ...
    std::stringstream ss;
    ss << (express_flipped ? "~" : "") << (anno ? "0x" : "") << std::hex << ull << std::dec;

    std::string s = ss.str();
    return s;
}

template <typename T> std::string SBit::DecString(T v, bool anno) // static
{
    std::bitset<64> bs(v);
    assert(bs.size() == 64);
    bool express_flipped = 2 * bs.count() > bs.size(); // more than half the bits are set
    ...
    std::stringstream ss;
    ss << (express_flipped ? "~" : "") << (anno ? "0d" : "") << std::dec << ull;

    std::string s = ss.str();
    return s;
}

template <typename T> std::string SBit::PosString(T v, char delim, bool anno) // static
{
    std::bitset<64> bs(v);
    assert(bs.size() == 64);
    ...
    if (bs.count() == 0)
        ss << delim; // no-bits set : still output delim as blank is too difficult to recognize as a value
    std::string s = ss.str();
    return s;
}
...
const char *SBit::ParseAnnotation(bool &complement, char &anno, const char *str_)
{
    ...
    anno = strlen(str_ + postcomp) > 2 && str_[postcomp] == '0' && strchr(ANNO, str_[postcomp + 1]) != nullptr
               ? str_[postcomp + 1]
               : '_';
...
#ifdef DEBUG
    std::cout << "SBit::FromPosString" << " str_[" << str_ << "]" << " str[" << str << "]" << " anno " << anno
              << " ull " << ull << std::endl;
#endif
...
unsigned long long SBit::FromEString(const char *ekey, const char *fallback)
{
    ...
    if (0)
        std::cout << "SBit::FromEString" << " ekey[" << (ekey ? ekey : "-") << "]\n"
                  << " str[" << (str ? str : "-") << "]\n"
                  << "\n";
...
unsigned long long SBit::FromString(const char *str)
{
    ...
    if (!anno_expect)
        std::cout << "SBit::FromString unexpected anno " << anno << " from str " << str << std::endl;
    assert(anno_expect);
    ...
    else
    {
        ...
        {
        case 'x':
            ull = FromHexString(str);
            break;
        case 'b':
            ull = FromBinString(str);
            break;
        case 'd':
